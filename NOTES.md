...
## The gauge does not forecast, deliberately

I built a turn-end Heat projection — hatched ticks showing where the stance
would leave the gauge, and a warning line before it crossed — and Robin took it
straight back out. Reverted whole.

**The ruling: keeping track of IAI's +2 is the player's job.** The stance strip
states the rule, the gauge states the number, and putting them together is part
of playing IAI rather than a chore to be automated away. Recorded here because
the argument for building it was a good one and I will otherwise make it again:
the gauge shows 6, the threshold is 8, the stance adds 2 at turn end, and it
looks like an information gap of exactly the kind the enemy telegraphs exist to
close.

It is not the same thing. An enemy intent is information the player *cannot*
derive — it lives in the enemy's script and nothing on screen implies it. IAI's
+2 is printed on the stance strip, in the same fight, at all times. Doing the
addition is knowledge, and this game is willing to ask for knowledge.

The same reasoning applies to the timing question that prompted it: charging the
Heat at turn start would remove the arithmetic by moving the payment, and the
answer there is the same. See the `heat.ts` note on the hard cap for where the
